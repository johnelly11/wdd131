...
function createArticles(articles) {
    ...
    articles.forEach(article => {
        ...
        articleDiv.classList.add('book-view');

        const html = `
            <div class="rating">
                <p id="release-date" aria-label="release date">${article.date}</p>
                <p aria-label="age rating">${article.ages}</p>
                <p aria-label="genre">${article.genre}</p>
                <p aria-label="rating">${article.stars}</p>
            </div>
            <div class="review">
                <h2>${article.title}</h2>
